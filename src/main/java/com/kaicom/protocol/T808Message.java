...
public class T808Message {
	...
	public final int getMessageType() {
		return getHeader().getMessageType()&0xffff;
	}
	...
	public final void ReadFromBytes(byte[] messageBytes) {
		...
		try {
			...
			if (xor == realXor) {
				...
				try {
					try {
						int tempLen = validMessageBytes.length - 1;
						byte[] headerBytes = new byte[tempLen < 16 ? 12 : 16];
						System.arraycopy(validMessageBytes, 0, headerBytes, 0,
								headerBytes.length);
